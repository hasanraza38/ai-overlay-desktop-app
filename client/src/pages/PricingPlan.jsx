...
//                     <button onClick={handleBack} className="p-1 rounded hover:bg-white/10 mr-4">
//                         <ArrowLeft className="w-4 h-4 text-white" />
//                     </button>
//                     <h1 className="text-3xl md:text-4xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-purple-500 to-purple-600">
//                         Boost Your Productivity with AI
//                     </h1>
//                 </div>
        ...
        const fetchUserAndPlans = async () => {
            try {
                ...
                if (userResponse.data.success) {
                    ...
                    // Set selected plan to user's current plan by default
    ...
    return (
        <div className="bg-black text-white min-h-screen mx-auto relative overflow-hidden">
            ...
            <div className="px-6 pb-8 mt-3">
                ...
                <div className="space-y-6 mb-8">
                    {plans.map((plan, idx) => (
                        <div
                            key={plan.id || idx}
                            onClick={() => setSelectedPlan(idx)}
                            className={`relative bg-gray-800 rounded-[12px] p-4 cursor-pointer transition-all duration-200 ${
                                selectedPlan === idx ? 'bg-gray-700 ring-2 ring-purple-500' : 'hover:bg-gray-700'
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-4">
                                    <div className={`w-6 h-6 rounded-[12px] border-2 flex items-center justify-center ${
                                        selectedPlan === idx ? 'bg-purple-600 border-purple-500' : 'border-gray-500'
                                    }`}>
                                        {selectedPlan === idx && <Check className="w-4 h-4 text-black" />}
                                    </div>
                                    <div>
                ...
                <button
                    onClick={purchasedPlan === selectedPlan ? null : handlePurchase}
                    className={`w-full font-semibold py-4 rounded-lg text-lg mt-2 transition-colors duration-200 focus:ring-opacity-50 cursor-pointer ${
